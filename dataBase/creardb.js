...
const manufacters = require("./products/dataManufacters");

const client = require("./conexion");

const createBBDD = async () => {
  const queryCreateProductsTable =
    "CREATE TABLE concesionario.public.manufacters( cif text NOT NULL, name text NOT NULL, addres text NOT NULL, PRIMARY KEY (cif) );";

  const queryCreateManufactersTable =
    "CREATE TABLE concesionario.public.products( id bigint NOT NULL, name text, manufacter_cif text, price bigint, color text, PRIMARY KEY (id), FOREIGN KEY (manufacter_cif) REFERENCES concesionario.public.manufacters (cif) );";

  try {
    await client.connect();
    await client.query(
      `${queryCreateProductsTable} ${queryCreateManufactersTable}`
    );
